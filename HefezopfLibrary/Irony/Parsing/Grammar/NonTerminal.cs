...
namespace Irony.Parsing
{
    ...
    public partial class NonTerminal : BnfTerm
    {
        ...
        {
            this._captionParameters = new IntList();
            this._convertedTemplate = this.NodeCaptionTemplate;
            var index = 0;
            while (index < 100)
            {
                var strParam = "#{" + index + "}";
                if (this._convertedTemplate.Contains(strParam))
                {
                    this._convertedTemplate = this._convertedTemplate.Replace(strParam, "{" + this._captionParameters.Count + "}");
                    this._captionParameters.Add(index);
                }
                if (!this._convertedTemplate.Contains("#{"))
                {
                    return;
                }
        ...
        {
            var paramValues = new string[this._captionParameters.Count];
            for (int i = 0; i < this._captionParameters.Count; i++)
            {
                var childIndex = this._captionParameters[i];
            ...
            var result = string.Format(this._convertedTemplate, paramValues);
            return result;
        }
        #endregion
