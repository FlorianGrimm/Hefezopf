...
namespace Irony.Parsing
{
    ...
    public class TokenPreviewHint : GrammarHint
    {
        ...
        public TokenPreviewHint(PreferredActionType actionType, string thisSymbol, params string[] comesBefore)
        {
            ...
            this._firstString = thisSymbol;
            this._beforeStrings.AddRange(comesBefore);
        }
        public TokenPreviewHint(PreferredActionType actionType, Terminal thisTerm, params Terminal[] comesBefore)
        {
            this._actionType = actionType;
            this._firstTerminal = thisTerm;
            this._beforeTerminals.UnionWith(comesBefore);
        }
        ...
        public override void Init(GrammarData grammarData)
        {
            ...
            this._firstTerminal = this._firstTerminal ?? this.Grammar.ToTerm(this._firstString);
            if (this._beforeStrings.Count > 0)
            {
                //SL pukes here, it does not support co/contravariance in full, we have to do it long way
                foreach (var s in this._beforeStrings)
                {
                    this._beforeTerminals.Add(this.Grammar.ToTerm(s));
                }
            }
            //Build description
            var beforeTerms = string.Join(" ", this._beforeTerminals.Select(t => t.Name));
            this._description = string.Format("{0} if {1} comes before {2}.", this._actionType, this._firstTerminal.Name, beforeTerms);
        }
        ...
        public override string ToString()
        {
            ...
            {
                this._description = this._actionType.ToString() + " if ...";
            }
        ...
        public override void Apply(LanguageData language, Construction.LRItem owner)
        {
            ...
            switch (this._actionType)
            {
                case PreferredActionType.Reduce:
                    ...
                    var reduceCondEntry = new ConditionalEntry(this.CheckCondition, reduceAction, this._description);
                ...
                case PreferredActionType.Shift:
                    ...
                    var shiftCondEntry = new ConditionalEntry(this.CheckCondition, shiftAction, this._description);
        ...
        private bool CheckCondition(ParsingContext context)
        {
            ...
            try
            {
                ...
                {
                    if (token.Terminal == this._firstTerminal)
                    {
                        return true; //found!
                    }

                    if (this._beforeTerminals.Contains(token.Terminal))
                    {
                        return false;
                    }
