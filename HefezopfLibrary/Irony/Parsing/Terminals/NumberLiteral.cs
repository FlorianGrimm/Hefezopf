...
namespace Irony.Parsing
{
    ...
    public class NumberLiteral : CompoundTerminalBase
    {
        ...
        public void AddExponentSymbols(string symbols, TypeCode floatType)
        {
            ...
            {
                this._exponentsTable[exp] = floatType;
            }
        }
        #endregion
        ...
        public override void Init(GrammarData grammarData)
        {
            ...
            if (this._exponentsTable.Count == 0 && !this.IsSet(NumberOptions.IntOnly))
            {
                this._exponentsTable['e'] = this.DefaultFloatType;
                this._exponentsTable['E'] = this.DefaultFloatType;
            }
        ...
        public override IList<string> GetFirsts()
        {
            ...
            {
                result.Add(this.DecimalSeparator.ToString());
            }
        ...
        protected override bool ReadBody(ISourceStream source, CompoundTokenDetails details)
        {
            ...
            while (!source.EOF())
            {
                ...
                bool isDot = current == this.DecimalSeparator;
                if (allowFloat && isDot)
                {
                    ...
                    if (digits.IndexOf(source.NextPreviewChar) < 0 && !this.IsSet(NumberOptions.AllowStartEndDot))
                    {
                        break; //from while loop
                    }
                ...
                bool isExpSymbol = (details.ExponentSymbol == null) && this._exponentsTable.ContainsKey(current);
                ...
                switch (typeCode)
                {
                    ...
                    case TypeCode.UInt64:
                        if (details.Value == null)
                        {
                            ...
                            this.TryConvertToLong(details, typeCode == TypeCode.UInt64); //try to convert to Long/Ulong and place the result into details.Value field;
                        }
        ...
        private void AssignTypeCodes(CompoundTokenDetails details)
        {
            ...
            {
                TypeCode code;
                if (this._exponentsTable.TryGetValue(details.ExponentSymbol[0], out code))
                {
        ...
        {
            int radix = this.GetRadix(details);
        ...
        private bool ConvertToFloat(TypeCode typeCode, CompoundTokenDetails details)
        {
            ...
            if (details.IsSet((short)NumberFlagsInternal.HasDot) && this.DecimalSeparator != '.')
            {
                body = body.Replace(this.DecimalSeparator, '.');
            }
        ...
        private bool ConvertToBigInteger(CompoundTokenDetails details)
        {
            ...
            int radix = this.GetRadix(details);
            int wordLength = this.GetSafeWordLength(details);
            int sectionCount = this.GetSectionCount(bodyLength, wordLength);
