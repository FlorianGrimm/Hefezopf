        ...
        internal HefezopfServiceApplication ServiceApplication
        {
            get
            {
                if (this.serviceApplication == null)
                {
                    ...
                    {
                        SPUtility.TransferToErrorPage(HttpContext.GetGlobalResourceObject("Hefezopf.Service.ServiceAdminResources", "ErrorNoServiceApplicationId", CultureInfo.CurrentCulture).ToString());
                    }

                    if (!GlobalAdminPageBase.IsValidGuid(serviceApplicationId))
                    {
                        SPUtility.TransferToErrorPage(HttpContext.GetGlobalResourceObject("Hefezopf.Service.ServiceAdminResources", "ErrorInvalidServiceApplicationId", CultureInfo.CurrentCulture).ToString());
                    }
